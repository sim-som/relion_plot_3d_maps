...
def parse_arguments():
    ...
    parser.add_argument('--map_pattern', type=str, default="run_class00?.mrc",
                        help='Glob pattern to find filtered maps (default: run_class00?.mrc)')
...
def main():
    ...
    # Find filtered maps
    filtered_maps = list(job_dir.glob(args.map_pattern))
    assert len(filtered_maps) > 0, f"No filtered maps found in {job_dir} with pattern {args.map_pattern}."
    ...
    # Load particle data
    particles_star_fpath = job_dir / Path("run_data.star")
    ...
    model_star_fpath = job_dir / Path("run_model.star")
